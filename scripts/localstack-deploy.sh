...
# Note: *.localhost.localstack.cloud DNS resolves to 127.0.0.1 via public DNS.
...
S3_BUCKET=$(node -e "console.log(require('$OUTPUTS_FILE')['$STACK_NAME'].S3BucketName)")
SQS_MESSAGE_QUEUE_URL=$(node -e "console.log(require('$OUTPUTS_FILE')['$STACK_NAME'].MessageQueueUrl)")

if [ -z "$S3_BUCKET" ] || [ "$S3_BUCKET" = "undefined" ]; then
  printf "  ${RED}Failed to extract S3BucketName from CDK outputs${NC}\n"
  exit 1
fi
if [ -z "$SQS_MESSAGE_QUEUE_URL" ] || [ "$SQS_MESSAGE_QUEUE_URL" = "undefined" ]; then
  printf "  ${RED}Failed to extract MessageQueueUrl from CDK outputs${NC}\n"
  exit 1
fi
