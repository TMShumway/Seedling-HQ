...
# Usage:
#   bash scripts/s3-ls.sh              # List all objects in the CDK-deployed bucket
...
# Read bucket name from .env.localstack (auto-generated by localstack-deploy.sh)
ENV_FILE="$REPO_ROOT/.env.localstack"
if [[ -f "$ENV_FILE" ]]; then
  BUCKET=$(grep '^S3_BUCKET=' "$ENV_FILE" | cut -d= -f2)
fi
BUCKET="${BUCKET:-fsa-local-local-uploads}"
